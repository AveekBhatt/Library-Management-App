...
const MyBooks = () =>{
     ...
     const [books , setbooks] = useState([])
       ...
       const showbooks = async() =>{
          try{
             const response = await axiosInstance.get("/showborrowed");
       ...
       const returnbooks = async(bookId) =>{
            try{
             const response = await axiosInstance.put("/returnbook/" + bookId);
             console.log(response.data)
             setbooks(response.data.books)
          }catch(error){
             if(error.response && error.response.data && error.response.data.message){
        ...
        const onSearchnote = async(query) =>{
            try{
               const response = await axiosInstance.get("/search-note-borrow",{
                 params :{query}
               })
              ...
              <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
                ...
                <div className="px-5 pb-5">
                 ...
                 <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 flex flex-col justify-between">
                  ...
                  <span className="text-xl font-bold text-gray-900 dark:text-white">Borrowed</span>
                  <button className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={()=>returnbooks(item._id)}>
                   Return
                  </button>
                  </div>
                 </div>
